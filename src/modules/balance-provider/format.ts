...
const formatIntegration = (integration: IntegrationBalance, opt: RecordOptions = {}): IntegrationBalance => {
    ...
    if (integration.integrationId) {
        integration.id = `${chain}:integration__${integration.integrationId}`;
    }

    if (!integration.balances.length) {
        return integration;
    }

    for (const balanceRecord of integration.balances) {
        formatRecord(Type.tokens, balanceRecord as AssetBalance, { chain, store, logo });
    }
...
const formatRecord = (type: BalanceType, record: AssetBalance, opt: RecordOptions = {}): AssetBalance => {
    ...
    if (record.symbol.startsWith('IBC.')) {
        record.symbol = record.symbol.replace('IBC.', '') || record.symbol;
    }

    if (DP_COSMOS[chain] && !record.balanceType) {
        record = cosmosChainTokens(record, opt);
    }
