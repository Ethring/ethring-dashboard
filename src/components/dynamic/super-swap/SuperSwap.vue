...
export default {
    ...
    setup() {
        ...
        const handleOnSelectToken = async (token, direction) => {
            ...
            if ((token && token.address && !token?.price) || isPriceUpdate) {
                const selectedNetwork = direction === TOKEN_SELECT_TYPES.FROM ? selectedSrcNetwork.value : selectedDstNetwork.value;

                token.price = await getPriceFromProvider(token.address, selectedNetwork, { coingeckoId: token.coingecko_id });
